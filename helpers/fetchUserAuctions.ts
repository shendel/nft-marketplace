...
const fetchUserAuctions = (options) => {
  const {
    address,
    userAddress,
    chainId,
    ...
    if (chainInfo && chainInfo.rpcUrls) {
      try {
        ...
        callMulticall({
          ...
          encoder: abiI,
          calls: {
            userOffers: { func: 'getUserAuctions', args: [ userAddress ] },
            timestamp: {
          ...
          resolve({
            ...
            userAddress,
            ...mcAnswer,
            userOffers: Web3ObjectToArray(mcAnswer.userOffers)
          })
        }).catch((err) => {
...
export default fetchUserAuctions